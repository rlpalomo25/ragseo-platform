...
from app.services.external_data import _latest_export_ids, build_market_context
...
def test_latest_export_ids_picks_newest_per_type(db_session):
    ...
    latest = _latest_export_ids(db_session, "kleangutter.com")

    assert latest["search_console"] == new.id
    assert latest["ai_overview"] == ai.id
    assert old.id not in latest.values()
    assert _latest_export_ids(db_session, "unknown.com") == {}


def test_latest_export_ids_tie_breaks_deterministically(db_session):
    ...
    first = _latest_export_ids(db_session, "leaffilter.com")["keyword_estimate"]

    assert first in (a.id, b.id)
    assert _latest_export_ids(db_session, "leaffilter.com")["keyword_estimate"] == first
...
def test_market_context_only_shows_latest_gsc_export(db_session):
    ...
    db_session.add(m.SearchConsoleDim(
        export_id=old.id, domain="kleangutter.com", dim_type="query",
        key="outdated query", clicks=1, impressions=900000))
    db_session.add(m.SearchConsoleDim(
        export_id=new.id, domain="kleangutter.com", dim_type="query",
        key="fresh query", clicks=1, impressions=123))
    db_session.commit()
...
def test_competitor_snapshot_scoped_to_latest_keyword_export(db_session):
    ...
    db_session.add(m.KeywordEstimate(
        export_id=old.id, domain="leaffilter.com",
        keyword="old competitor kw", position=2.0, est_visits=9999))
    db_session.add(m.KeywordEstimate(
        export_id=new.id, domain="leaffilter.com",
        keyword="new competitor kw", position=1.0, est_visits=7))
    db_session.commit()
...
def test_brand_health_scoped_to_latest_report_export(db_session):
    ...
    db_session.add(m.DomainMetric(export_id=old.id, domain="kleangutter.com",
                                  metric="domain_authority", value=41))
    db_session.add(m.DomainMetric(export_id=new.id, domain="kleangutter.com",
                                  metric="domain_authority", value=55))
    db_session.commit()
