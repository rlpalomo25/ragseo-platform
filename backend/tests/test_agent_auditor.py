...
def _verdict_response(verdict="pass_with_notes", findings=None):
    return json.dumps({
        ...
        "findings": findings or [
            {"check": "Above-fold questions", "severity": "minor",
             "status": "warn", "notes": "Only two teasers present."},
        ],
    })
...
def test_auditor_coerces_non_standard_status(doctrine_corpus, monkeypatch):
    findings = [{
        "check": "Meta keywords", "severity": "major",
        "status": "flag", "notes": "Prioritize primary keyword.",
    }]
    monkeypatch.setattr(
        auditor_module, "call_llm",
...
def test_auditor_critical_fail_forces_fail_verdict(doctrine_corpus, monkeypatch):
    findings = [{
        "check": "Structure deviation", "severity": "critical",
        "status": "fail", "notes": "H2 order violates Doc 102.",
    }]
...
def test_auditor_scopes_packaging_criticals(doctrine_corpus, monkeypatch):
    findings = [{
        "check": "JSON-LD in both forms (Doc 192 7A)", "severity": "critical",
        "status": "fail", "scope": "packaging", "notes": "No script tag.",
    }]
    monkeypatch.setattr(
        auditor_module, "call_llm",
...
def test_auditor_content_critical_still_fails_with_packaging(doctrine_corpus, monkeypatch):
    findings = [
        {"check": "Stylesheet", "severity": "critical", "status": "fail",
         "scope": "packaging", "notes": "Missing style block."},
        {"check": "Claim trust level", "severity": "critical", "status": "fail",
         "scope": "content", "notes": "No Level 2 claim sourced."},
    ]
    monkeypatch.setattr(
        auditor_module, "call_llm",
        lambda **kw: _verdict_response(verdict="fail", findings=findings),
...
def test_auditor_unknown_scope_defaults_content(doctrine_corpus, monkeypatch):
    findings = [{
        "check": "Manifest", "severity": "critical", "status": "fail",
        "scope": "artifact", "notes": "Missing.",
    }]
    monkeypatch.setattr(
        auditor_module, "call_llm",
        lambda **kw: _verdict_response(verdict="fail", findings=findings),
...
def test_auditor_reads_writer_task_output(doctrine_corpus, monkeypatch):
    task = AgentTask(
        ...
        output_data={"agent": "writer", "provenance": [],
                     "output": {"title": "T", "meta_title": "MT", "meta_description": "MDesc",
                                "content_markdown": "# T\n\nDraft body."}},
...
def test_auditor_missing_input_raises(doctrine_corpus):
    with pytest.raises(ValueError, match="content.*source_task_id"):
