...
def retrieval_corpus(db_session):
    ...
    for idx, (doc, text) in enumerate(
        [
            (doc100, "content rules for page structure"),
...
def test_retrieve_with_doc_numbers_filters_docs(retrieval_corpus):
    ...
    results = retrieve(retrieval_corpus, "content structure order", top_k=10, doc_numbers=["100", "316"])
    assert results
    doc_numbers = {r.doc_number for r in results}
...
def test_retrieve_with_single_doc_number(retrieval_corpus):
    ...
    results = retrieve(retrieval_corpus, "content structure order", top_k=10, doc_numbers=["316"])
    assert results
